...
var arraySortingUtilities = require('array-sorting-utilities');
var intervalUtilities = require('interval-utilities');
var DataIntervalTree = require('node-interval-tree');
var pointUtilities = require('point-utilities');
var equal = require('deep-equal');

function _interopDefault (e) { return e && e.__esModule ? e : { 'default': e }; }

var DataIntervalTree__default = /*#__PURE__*/_interopDefault(DataIntervalTree);
...
if (react.unstable_getCacheForType == null) {
...
function createCache(options) {
    ...
    getKey = params => {
      ...
      if (!didLogWarning) {
        if (key.includes("[object Object]")) {
          didLogWarning = true;
          console.warn(`Warning: createCache() key "${key}" contains a stringified object and may not be unique`);
        }
      }
      return key;
    };
  ...
  const getCacheForType = immutable ? () => recordMap : react.unstable_getCacheForType;
...
function createIntervalCache(options) {
  ...
  const arraySortUtils = arraySortingUtilities.configure((a, b) => comparePoints(getPointForValue(a), getPointForValue(b)));
  const intervalUtils = intervalUtilities.configure(comparePoints);
  const pointUtils = pointUtilities.configure(comparePoints);
  ...
  function subscribe(callback, start, end, ...params) {
    ...
    if (tree == null) {
      tree = new DataIntervalTree__default["default"]();
          ...
          if (progress != null) {
            {
              if (progress < 0 || progress > 1) {
                console.warn(`Invalid progress: ${progress}; value must be between 0-1.`);
              }
            }
...
function useCacheMutation(cache) {
  ...
  const mutateSync = react.useCallback((params, newValue) => {
    const cacheKey = getKey(params);
    if (mutationAbortControllerMap.has(cacheKey)) {
      const abortController = mutationAbortControllerMap.get(cacheKey);
      abortController.abort();
  ...
  const mutateAsync = react.useCallback(async (params, callback) => {
    const cacheKey = getKey(params);
    if (mutationAbortControllerMap.has(cacheKey)) {
      const abortController = mutationAbortControllerMap.get(cacheKey);
      abortController.abort();
...
function useCacheStatus(cache, ...params) {
  return react.useSyncExternalStore(callback => cache.subscribe(callback, ...params), () => cache.getStatus(...params), () => cache.getStatus(...params));
}
...
function useImperativeCacheValue(cache, ...params) {
  ...
  const [prevParams, setPrevParams] = react.useState(undefined);
      ...
      case STATUS_RESOLVED:
        {
          ...
          setPrevParams(prevParams => {
            if (prevParams == params || equal__default["default"](prevParams, params)) {
              return prevParams;
            } else {
              return params;
            }
          ...
          setPrevValue(prevValue => {
            if (prevValue == value || equal__default["default"](prevValue, value)) {
              return prevValue;
            } else {
              return value;
            }
  ...
  switch (status) {
    ...
    case STATUS_PENDING:
      {
        value = prevParams == params || equal__default["default"](prevParams, params) ? prevValue : undefined;
        break;
      }
  }
...
function useIntervalCacheStatus(cache, start, end, ...params) {
  return react.useSyncExternalStore(callback => cache.subscribe(callback, start, end, ...params), () => cache.getStatus(start, end, ...params), () => cache.getStatus(start, end, ...params));
}
...
function useStreamingValue(streamingValues, options = {}) {
  ...
  const throttledSubscribe = react.useCallback(callback => {
    const callbackWrapper = throttle(() => {
      callback();
    }, throttleUpdatesBy);
    return streamingValues.subscribe(callbackWrapper);
  }, [streamingValues.subscribe]);
  return react.useSyncExternalStore(throttledSubscribe, getValue, getValue);
}
...
exports.STATUS_ABORTED = STATUS_ABORTED;
...
exports.assertPendingRecord = assertPendingRecord;
exports.assertPendingRecordData = assertPendingRecordData;
exports.assertRejectedRecord = assertRejectedRecord;
exports.assertRejectedRecordData = assertRejectedRecordData;
exports.assertResolvedRecord = assertResolvedRecord;
exports.assertResolvedRecordData = assertResolvedRecordData;
exports.createCache = createCache;
exports.createDeferred = createDeferred;
exports.createExternallyManagedCache = createExternallyManagedCache;
exports.createInfallibleCache = createInfallibleCache;
exports.createIntervalCache = createIntervalCache;
exports.createPendingRecord = createPendingRecord;
exports.createPendingRecordData = createPendingRecordData;
exports.createRejectedRecord = createRejectedRecord;
exports.createRejectedRecordData = createRejectedRecordData;
exports.createResolvedRecord = createResolvedRecord;
exports.createResolvedRecordData = createResolvedRecordData;
exports.createSingleEntryCache = createSingleEntryCache;
exports.createStreamingCache = createStreamingCache;
exports.disableDebugLogging = disableDebugLogging;
exports.enableDebugLogging = enableDebugLogging;
exports.isPendingRecord = isPendingRecord;
exports.isPendingRecordData = isPendingRecordData;
exports.isPromiseLike = isPromiseLike;
exports.isRejectedRecord = isRejectedRecord;
exports.isRejectedRecordData = isRejectedRecordData;
exports.isResolvedRecord = isResolvedRecord;
exports.isResolvedRecordData = isResolvedRecordData;
exports.log = log;
exports.parallelize = parallelize;
exports.updateRecordToPending = updateRecordToPending;
exports.updateRecordToRejected = updateRecordToRejected;
exports.updateRecordToResolved = updateRecordToResolved;
exports.useCacheMutation = useCacheMutation;
exports.useCacheStatus = useCacheStatus;
exports.useImperativeCacheValue = useImperativeCacheValue;
exports.useImperativeIntervalCacheValues = useImperativeIntervalCacheValues;
exports.useIntervalCacheStatus = useIntervalCacheStatus;
exports.useStreamingValue = useStreamingValue;
